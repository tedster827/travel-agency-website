...
const NavigationBarMenu: React.FunctionComponent<NavigationBarMenuProps> = ({
  menuName,
  listOfMenuItems,
  ...
  const toggleDropdown = (isVisible: boolean) => {
    ...
    if (isVisible) {
      ...
      }, 1_000); // Timeout for 1 seconds (1000 milliseconds)
    }
  };
            ...
            {listOfMenuItems.map((menuItem: MenuItem): ReactNode => {
              return (
                <li key={menuItem.name}>
                  <Link href={!menuItem.linkPath ? "/" : menuItem.linkPath}>
